...
Considering this code will eventually be run on an NVIDIA Jetson Orin Nano Super an embedded system chip mounted on the RoboRacer vehicle.
...
The Jetson has a good amount of compute on the GPU and therefore should be explored by taking advantage of GPU offloading.
This is a common occurrence in neural networks, as they have an abundance of matrix-vector multiplications and other high data intensive operations which the GPU is more suitable for given its large number of core counts compared to the CPU.
Hence, the central idea hinged on taking the vectorized code from previous sections \ref{section:VecSampling} and \ref{section:VecWallCollision} and utilizing PyTorch's device context manager to transfer requested operations to the GPU.
...
However, in order to solve the LQR of DiffMPC that required meticulous sequential ordering as the output of the previous solution was used to solve the next time step.
...
Leading to overall a worse execution time for the GPU offloading approach.
