...
The agent learns a policy by interacting with its environment and receiving rewards based on actions which align with the desired behavior \cite{wang_VersatileEfficientReinforcement_2022, chib_recent_2023}.
...
Because the agent actively explores its state-action space, RL is robust to the distribution-shift issue inherent to IL \cite{chib_recent_2023, wang_VersatileEfficientReinforcement_2022}.
...
%* check issue
Furthermore, scaling RL to high-dimensional sensory inputs such as raw camera or LiDAR streams remains notoriously difficult \cite{wang_VersatileEfficientReinforcement_2022}.
...
%* rewrite this to have shorter sentences, and include high volume data collection or large training time to be included.
Common to both paradigms is the recurring concern that the resulting policies remain difficult to interpret and to certify against explicit safety requirements, as their decision-making is encoded implicitly within the weights of a deep network rather than in human-readable rules \cite{tampuu_SurveyEndtoEndDriving_2022, wang_VersatileEfficientReinforcement_2022, hu_review_2026, jain_Autonomy_20_2021, jahncke_MINDStackModularInterpretable_2025}.
This opacity, combined with the data and compute demands outlined above, motivates the hybrid differentiable architectures discussed in the following section, which seek to retain the structural transparency of the modular stack while still benefiting from learned, jointly optimized components.
