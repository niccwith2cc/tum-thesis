...
This chapter presents a more detailed and technical description of previous approaches, emphasizing the shortcomings that would eventually motivate a more universal yet safe implementation of autonomous navigation systems.
...
The software pipeline was described as comprising of approximately 30 modules organized into six functional layers \cite{tampuu_SurveyEndtoEndDriving_2022, thrun_stanley_2006, montemerlo_winning_2006}.
...
Convolution Neural Networks (CNNs) are commonly paired with IL models, as they are well suited to processing the high-dimensional visual data produced by the vehicle's cameras \cite{bojarski_EndtoEndLearningSelfDriving_2016, kocic_end-to-end_2019, lee_autonomous_2020, hu_learning_2021}.
...
Additionally, there have been several varying degrees of hybridization between the modular and end-to-end neural network approaches.
...
Hu et al. \cite{hu_planning-UniAD_2023} propose Unified Autonomous Driving (UniAD), a planning-oriented stack that utilizes the standard decomposition of tasks: perception, prediction, and planning but trains every stage inside one transformer network \cite{vaswani_AttentionAllYou_2017}.
The authors rearrange the pipeline so that intermediate outputs exist primarily to support the final ego-planning decision.
...
Four transformer decoders are then used to interpret that representation in the following order.
...
UniAD couples modules via shared query interfaces, and makes use of the waypoint's planning loss which are backpropagates through MotionFormer, OccFormer, and the upstream perception heads.
TrackFormer and MapFormer are further rewarded to produce features that help planning rather than maximize isolated perception scores \cite{hu_planning-UniAD_2023}.
Notably, the gradient path does not proceed beyond the learned planning decoder and therefore is only contained to improve the planning phase.
Fortunately, the planning-centric methodology does show to be effective on the multimodal dataset nuScenes \cite{caesar_nuScenesMultimodalDataset_2020}.
...
Results additionally show that adding motion forecasting and occupancy prediction before the planner steadily improves both metrics, and UniAD also matches or beats task-specific baselines on tracking, online mapping, motion forecasting, and occupancy prediction despite not targeting those metrics directly \cite{hu_planning-UniAD_2023}.

UniAD nevertheless remains only a partial instance of the differentiable modular stack pursued in this thesis.
The planner predicts positions without explicit velocity, yaw, or dynamic feasibility constraints.
Collision reasoning also relies on static occupancy proximity rather than time-to-collision method.
Hence, the optimization stack never reaches the controller level and is not able to collect information at that stage to backpropagate.
In short, UniAD demonstrates that planning-oriented joint training across neural modules is valuable, but it cannot capture sensor-to-actuator, optimization-aware differentiability.
...
Karkus et al. \cite{karkus_DiffStackDifferentiableModular_2022} take an opposing emphasis from UniAD.
DiffStack keeps explicit prediction, planning, and control blocks and makes each one differentiable enough for joint training.
The prediction module is defined as a graph-structured Conditional Variational Autoencoder (CVAE).
...
The CVAE samples a latent variable and decodes multiple future trajectory possibilities with associated probabilities; both steps stay differentiable so downstream losses can reach the encoder weights.
...
Finally, The control module wraps up with box-constrained iterative Linear Quadratic Regulator (iLQR) MPC \cite{karkus_DiffStackDifferentiableModular_2022}.
iLQR linearizes dynamics, solves a local LQR sub-problem, and iterates until convergence.
Implicit differentiation is achieved through the final sub-problem's Karush-Kuhn-Tucker (KKT) conditions, following Amos et al. \cite{amos_OptNetDifferentiableOptimization_2017, karkus_DiffStackDifferentiableModular_2022}.
Hence, achieving analytical gradients of the controlled trajectory with respect to $w$ and, through the collision-cost terms, with respect to the predicted agent states.

Training was conducted on nuScenes \cite{caesar_nuScenesMultimodalDataset_2020} by aiming to minimize the loss $\mathcal{L} = \mathcal{L}_{\text{pred}} + \mathcal{L}_{\text{plan}} + \mathcal{L}_{\text{ctrl}}$.
The researchers combined a forecasting loss, a cross-entropy that pulls the soft planner selection toward the ground-truth candidate, and a control loss that measures imitation error after iLQR \cite{karkus_DiffStackDifferentiableModular_2022}.
They found that joint optimization reshapes the predictor toward planning-relevant forecasts.
Where DiffStack reports up to roughly 15\% higher prediction effectiveness for planning and reduces final control cost by up to 15.5\% compared with separately trained modules \cite{karkus_DiffStackDifferentiableModular_2022}.

However, there exists discontinuities in the gradient path of DiffStack which hold it back from reaching full differentiability.
The planner output is explicitly detached before initializing iLQR. 
Consequently, control-loss gradients reach the prediction module by bypassing the planner rather than flowing through it \cite{karkus_DiffStackDifferentiableModular_2022}.
Therefore, the cost weights $w$ are aware of planning-loss gradients but not control-loss gradients, and the softmax still relaxes only the choice among a finite sample set rather than differentiating the sampling process itself.
Additionally, DiffStack assumes clean construction of the state estimate using its sensors and agent tracker, so the perception module sits entirely outside of the stack.
Finally, only the weight vector $w$ is learned, the cost terms and vehicle model remain hand-specified.
DiffStack thus spans prediction through control with interpretable modules, yet it is unable to reach the full capacity of its optimization through the limited gradient path.
...
Huang et al. \cite{huang_DifferentiableIntegratedMotion_2024} address the coupling between prediction and planning with Differentiable Integrated Motion Prediction and Planning (DIPP).
Where previously DiffStack relaxes discrete trajectory selection inside a sampler to achieve differentiability.
DIPP embeds a differentiable nonlinear optimizer directly in the system, treating the planner's cost weightings as learnable parameters.

DIPP processes a vectorized scene made up of past ego and agent state histories alongside an HD-map lane graph \cite{huang_DifferentiableIntegratedMotion_2024}.
A transformer encoder combines each agent trajectory and map polyline into a unified representation, using self-attention over agents and cross-attention to map elements \cite{vaswani_AttentionAllYou_2017}.
A multi-modal decoder predicts a small set of future trajectories and mode probabilities for every surrounding agent, while proposing an initial ego plan that seeds the optimizer \cite{huang_DifferentiableIntegratedMotion_2024}.
...
\end{equation}
where each $c_i$ is a squared residual for progress, comfort, safety relative to the predicted trajectories $\hat{s}^{\text{pred}}_{1:T}$, or rule compliance.
Each scalar weight $\omega_i$ is learned from data \cite{huang_DifferentiableIntegratedMotion_2024}.
A Gauss-Newton solver iterates on the linearized residuals until convergence.
The researchers also noted that the residuals and the solver are differentiable, hence, backpropagation can reach the predicted agent states and the weights $\omega_i$ by implicitly differentiating through the solver's fixed point \cite{huang_DifferentiableIntegratedMotion_2024}.

Notably, end-to-end training minimizes an imitation loss between the optimized ego trajectory and logged human driving, alongside a forecasting loss for surrounding agents \cite{huang_DifferentiableIntegratedMotion_2024}.
A unified backward pass jointly updates the scene encoder-decoder, the prediction heads, and the planner's cost weights $\omega_i$.
Consequently, gradients backpropagated through the optimizer refine the predicted trajectories to directly support motion planning, a mechanism the authors call planning-centric prediction.
On the Waymo Open Motion Dataset, DIPP outperforms both modular baselines and pipelines utilizing independently trained predictors, demonstrating improvements in minADE and collision rates  \cite{huang_DifferentiableIntegratedMotion_2024}.

DIPP extends modular differentiable planning by integrating the planner's cost formulation directly into the gradient path. 
However, several architectural limitations persist at the stack boundaries.
The perception and map vectorization remain non-differentiable upstream components, execution lacks a low-level controller to convert planned trajectories into actuator commands, and the cost residuals $c_i$ rely on fixed functional forms where only the scalar weights $\omega_i$ are learnable.
DIPP therefore still offers partial rather than full sensor-to-actuator differentiability.
...
Jahncke and Betz \cite{jahncke_MINDStackModularInterpretable_2025} introduce the Modular, Interpretable, and Differentiable (MIND) software stack, which shifts the autonomy requirement to the RoboRacer autonomous racing platform \cite{betz_TUMAutonomousMotorsport_2023, okelly_F1TENTHOpensourceEvaluation_2019}.
MIND is formulated to address a gap left by systems such as UniAD, DiffStack, and DIPP.
Albeit, those frameworks show that joint optimization across modules is beneficial, they limit the number of modules included in this optimization.
This achieves partial or incomplete optimization when looking throughout the entire stack. 
MIND instead builds a compact stack around two explicit modules, a learned localization network and a classical Stanley controller.
Hence, the authors setup the modules in a manner that a downstream control loss can train the upstream localization module.

The MIND pipeline begins with the sensor observations available on the RoboRacer vehicle and passes them through a localization CNN that estimates the ego pose relative to the track.
Hence, intermediate quantities such as lateral cross-track error $e_{\text{cross}}$ and heading error relative to the reference path $e_{\text{head}}$ are calculated from the ego pose \cite{jahncke_MINDStackModularInterpretable_2025}.
The Stanley controller uses these quantities to calculate a steering angle based on the following formula:
\begin{equation}
    \delta = k_h e_{\text{head}} + \arctan \left(\frac{k_e e_{\text{cross}}}{v}\right),
\end{equation}
where $\delta$ is the steering angle, $k_h$ is a tuning parameter for the heading error, $k_e$ is a tuning parameter for the sensitivity of the controller for the cross-track error, and $v$ represents the vehicle's current speed.
The simple differentiability of MIND comes from enabling all modules to be differentiable during training.
A control loss is defined that measures tracking error along the reference trajectory, and backpropagates this loss through the controller into the localization network \cite{jahncke_MINDStackModularInterpretable_2025}.
As a result, the localization module minimizes pose-estimation error and learns representations that reduce the steering and tracking mistakes the controller will produce on track.
As discussed previously, this approach relies on a different hybridization pattern than UniAD's neural task fusion and from DiffStack's gradient path. 
MIND couples raw sensor input to actuator control error across a two modules.
...
The authors also demonstrate sim-to-real transfer by deploying the jointly trained stack on the physical RoboRacer platform \cite{jahncke_MINDStackModularInterpretable_2025}.

MIND nevertheless remains only a partial realization of a full modular differentiable driving stack.
No inclusion of prediction, obstacle interaction, or a differentiable motion planner between perception and actuation.
Further limitation assumes the reference path to be available, and no local refinement plan is executed when racing.
These limitation directly motivate the subsequent DiffPlan work \cite{swadiryus_DifferentiableMotionPlanning_2025, koch_DifferentiableInterpretablePath_2025}, which extends the MIND philosophy by inserting a differentiable sampling-based motion planner.
Even in its present form, however, MIND is an important step as it proves that partial differentiability can be both interpretable and deployable, and provides the software foundations this thesis is built on.
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}
        ...
        % car and global heading
        \coordinate (car) at (0.05, 2);
        \fill[black] (car) circle (2pt);
        \draw[dashed, thin] (car) -- ++(0.7, 0);
        \draw[->, thick, blue] (car) -- ++(0, 0.75);
        \draw (0.35, 2) arc (0:90:0.3) node[midway, above right, inner sep=1pt, font=\small] {$\psi$};
...
The curve vehicles follow is called a raceline; Figure~\ref{fig:trackrep} illustrates a circuit boundary, an example raceline, the global Cartesian frame, and the heading angle $\psi$ measured from the $x$-axis.
It is a mathematically optimal line aiming to achieve a certain goal, most commonly fastest time to complete a full lap
...
Representative algorithms in this family include Dijkstra's algorithm, A*, and hybrid A*.
...
Hybrid A* further combines discrete graph search with continuous vehicle kinematics, connecting lattice nodes through dynamically feasible arcs so that the resulting path respects the vehicle's steering limits and minimum turning radius \cite{gonzalez_ReviewMotionPlanning_2016}.
...
The principal drawbacks of graph and search planners are tied to their discretization.
...
\begin{figure}[ht!]
    ...
    \begin{tikzpicture}[
        ...
        inner/.style={rectangle, draw, thick, rounded corners, align=center, font=\scriptsize},
        flow/.style={-{Stealth[length=2mm]}, very thick},
        grad/.style={-{Stealth[length=2mm]}, thick, dashed, gray},
        lbl/.style={font=\scriptsize, fill=white, inner sep=1pt},
        gradlbl/.style={font=\scriptsize, text=gray!70!black, fill=white, inner sep=1pt},
    ]
        % --- Perception: CNN only ---
        \node[comp, draw=yellow, fill=yellow!10, minimum width=2.8cm, minimum height=2.2cm] (perc) at (0, 1.6) {};
        \node[font=\small\bfseries] at (0, 2.45) {Perception};
        % Centered 1D-CNN icon (three stacked conv layers)
        \coordinate (cnn_base) at (-0.63, 0.95);
        \foreach \i in {0,1,2} {
            \draw[fill=yellow!45, draw=yellow!70!black, thick, rounded corners=0.5pt]
                ([xshift=\i*1.8mm, yshift=\i*1.2mm]cnn_base) rectangle
                ([xshift=\i*1.8mm+9mm, yshift=\i*1.2mm+5.5mm]cnn_base);
        }
        \node[font=\scriptsize] at (0, 0.65) {1D-CNN};

        % --- Prediction: empty shell below perception ---
        \node[comp, draw=orange, fill=orange!8, minimum width=2.8cm, minimum height=1cm] (pred) at (0, -0.6) {};
        \node[font=\small\bfseries] at (0, -0.6) {Prediction};

        % --- Planning ---
        \node[comp, draw=blue, fill=blue!8, minimum width=3.6cm, minimum height=3.8cm] (plan) at (6.2, 0.5) {};
        \node[font=\small\bfseries] at (6.2, 2.15) {Planning};
        \node[inner, draw=blue, fill=blue!15, minimum width=2.8cm, minimum height=0.65cm] (sample) at (6.2, 1.35) {Sample generation};
        \node[inner, draw=blue, fill=blue!15, minimum width=2.8cm, minimum height=0.65cm] (assess) at (6.2, 0.35) {Assessment};
        \node[inner, draw=blue, fill=blue!15, minimum width=2.8cm, minimum height=0.65cm] (select) at (6.2, -0.65) {Sample selection};
        \draw[flow, blue] (sample.south) -- (assess.north);
        \draw[flow, blue] (assess.south) -- (select.north);

        % --- Control ---
        \node[comp, draw=green, fill=green!8, minimum width=3.2cm, minimum height=2.8cm] (ctrl) at (11.0, 0.5) {};
        \node[font=\small\bfseries] at (11.0, 1.7) {Control};
        \node[inner, draw=green, fill=green!15, minimum width=2.4cm, minimum height=0.75cm] (diffmpc) at (11.0, 0.65) {DiffMPC};
        \node[inner, draw=green, fill=green!15, minimum width=2.4cm, minimum height=0.75cm] (motion) at (11.0, -0.45) {Motion model};
        \draw[flow, green!70!black] (diffmpc.south) -- (motion.north);

        % --- Loss ---
        \node[comp, draw=purple, fill=purple!10, minimum width=2.8cm, minimum height=0.75cm] (loss) at (11.0, -2.35) {Loss calculation};

        % --- Forward data flow ---
        \draw[flow, black!70] ($(perc.west)+(-2.3,0)$) -- node[lbl, above] {LiDAR scan} (perc.west);
        \draw[flow, yellow!70!black] (perc.east) -- node[lbl, above] {Pose} (sample.west);
        \draw[flow, orange] (pred.east) to[out=10, in=170]
            node[lbl, pos=0.42, above] {Obstacle locations} ($(sample.west)+(0,0.12)$);
        \draw[flow, orange] (pred.east) to[out=-10, in=190]
            node[lbl, pos=0.42, below] {Opponent waypoints} ($(sample.west)+(0,-0.12)$);
        \draw[flow, blue] (select.east) -- node[lbl, above] {Trajectory} (diffmpc.west);
        \draw[flow, green!70!black] (motion.south) -- (loss.north);

        % --- Gradient backpropagation ---
        % Every grad arrow is drawn parallel to its forward counterpart with a
        % consistent offset (vertical arrows shifted left of the midline,
        % horizontal arrows shifted below) so the dashed grey path stays clearly
        % separated from the solid forward flow.
        % (a) Loss -> motion model (offset left of the forward motion->loss arrow).
        \draw[grad] ($(loss.north)+(-0.3,0)$) -- ($(motion.south)+(-0.3,0)$);
        % (b) Motion model -> DiffMPC (offset left of internal control arrow).
        \draw[grad] ($(motion.north)+(-0.3,0)$) -- ($(diffmpc.south)+(-0.3,0)$);
        % (c) DiffMPC -> Sample selection (below the trajectory arrow).
        \draw[grad] ($(diffmpc.west)+(0,-0.25)$) --
            node[gradlbl, below] {$\nabla$ selection}
            ($(select.east)+(0,-0.25)$);
        % (d) Sample selection -> Assessment -> Sample generation (offset left of
        %     the internal planning arrows). Straight-Through-Gumbel-Softmin lets
        %     gradients flow across the discrete arg-min.
        \draw[grad] ($(select.north)+(-0.3,0)$) -- ($(assess.south)+(-0.3,0)$);
        \draw[grad] ($(assess.north)+(-0.3,0)$) -- ($(sample.south)+(-0.3,0)$);
        % (e) Sample generation -> Perception (below the Pose arrow) into DiffLoc.
        \draw[grad] ($(sample.west)+(0,-0.25)$) --
            node[gradlbl, below] {$\nabla\theta_j,\,\nabla\phi_{\text{DiffLoc}}$}
            ($(perc.east)+(0,-0.25)$);
        % (f) Direct localization loss supervision to perception. Routed above
        %     the diagram (right of control, over the top, down to perception)
        %     so it never crosses the forward pipeline.
        \draw[grad] (loss.east) -- (13.2,-2.35) -- (13.2, 3.4)
            -- node[gradlbl, above] {$\mathcal{L}_{\text{loc}}$} (0, 3.4)
            -- (perc.north);
    \end{tikzpicture}
    \caption{DiffPlan architecture with internal module layout.
    Solid arrows denote forward data flow.
    Dashed grey arrows trace gradient backpropagation through every differentiable stage: $\mathcal{L}_{\text{prog}}$, $\mathcal{L}_{\text{coll}}$, and $\mathcal{L}_{\text{rl}}$ flow backward through the motion model, DiffMPC, Gumbel-Softmin selection, assessment, and sample generation into DiffLoc, while $\mathcal{L}_{\text{loc}}$ provides a direct supervision path to perception; prediction is omitted from the gradient path because it supplies ground-truth environment data rather than learned estimates \cite{koch_DifferentiableInterpretablePath_2025, swadiryus_DifferentiableMotionPlanning_2025}.}
...
The term itself is the mean planned speed along the candidate trajectory,
\begin{equation}
    C_{\text{slow}} = \frac{1}{T}\sum_{t=0}^{T-1} V_t,
\end{equation}
and is combined with a negative trainable weight in the total cost sum, so that faster candidates receive a lower overall score.
This penalizes unnecessarily conservative plans, which is particularly useful during overtaking when the raceline reference velocity may be suboptimal.
...
At each waypoint $t$ of the candidate, the lateral clearance to the left and right track boundaries is computed from the interpolated track widths $w_{\text{L}}(s_t)$ and $w_{\text{R}}(s_t)$ and the lateral offset $n_t$,
\begin{equation}
    d_{\text{L}, t} = w_{\text{L}}(s_t) - n_t, \qquad d_{\text{R}, t} = n_t + w_{\text{R}}(s_t).
\end{equation}
To retain a differentiable objective, the nearest-wall distance is obtained via a softmin with temperature $\tau_w$,
\begin{equation}
    d_{\min, t} = \sum_{i \in \{\text{L}, \text{R}\}} \frac{\exp(-d_{i,t} / \tau_w)}{\sum_{j} \exp(-d_{j,t} / \tau_w)} \cdot d_{i, t}.
\end{equation}
A safety margin $d_{\text{safe}}$ then triggers a fourth-power penalty on the shortfall, weighted per waypoint by $\alpha_t$ (with early waypoints emphasized to prioritize near-term safety):
\begin{equation}
    \delta_t = \max(0,\; d_{\text{safe}} - d_{\min, t}), \qquad C_{\text{wall}} = \sum_{t=0}^{T-1} \alpha_t \, \delta_t^{\,4}.
\end{equation}
The exponent sharpens the penalty as the candidate approaches the boundary while remaining smoothly differentiable, encouraging the planner to maintain a safety buffer while still allowing learnable trade-offs between aggression and collision risk.

\subsubsection{Jitter cost}
To reduce path oscillations between consecutive planning cycles, Koch includes a jitter penalty that measures the deviation of each candidate from the trajectory selected in the previous planning step \cite{koch_DifferentiableInterpretablePath_2025}.
Let $(s^{\text{prev}}_t, n^{\text{prev}}_t)$ denote the Frenet coordinates of the previously chosen trajectory at waypoint $t$, and let $s_{\text{end}}$ be the track length. The waypoint-wise separation is
\begin{equation}
    \Delta s_t = \min\!\left(|s_t - s^{\text{prev}}_t|,\; s_{\text{end}} - |s_t - s^{\text{prev}}_t|\right), \qquad \Delta n_t = |n_t - n^{\text{prev}}_t|,
\end{equation}
where the wrap-around in $\Delta s_t$ handles the start/finish-line periodicity of the closed racetrack.
The jitter cost is then the sum of squared Frenet-plane distances,
\begin{equation}
    C_{\text{jit}} = \sum_{t=0}^{T-1} \left(\Delta s_t^{\,2} + \Delta n_t^{\,2}\right).
\end{equation}
...
\caption{Differentiable Sample Selection}
    ...
    \State $y_{\text{select}} \gets \operatorname{sg}[y_{\text{hard}} - y_{\text{soft}}] + y_{\text{soft}}$ \Comment{where $\operatorname{sg}[\cdot]$ stops gradients}
